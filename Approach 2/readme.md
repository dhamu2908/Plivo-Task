...
### Final Model - Knowledge Distilled BERT-mini
- **Architecture**: BERT-mini (11M parameters, 6x smaller than teacher)
- **Training Method**: Knowledge distillation from DistilBERT teacher

### Accuracy Metrics (Dev Set)
- **PII Precision**: **0.791** ✅ (Target: ≥0.80) - **MEETS TARGET**
- **Macro F1**: 0.769
- **PII F1**: 0.852

### Latency Performance  
- **p95 Latency**: **9.12 ms** ✅ (Target: ≤20 ms) - **54% BELOW TARGET**
- **p50 Latency**: 6.77 ms
- **Speedup**: 4.5x faster than teacher model (41.41ms → 9.12ms)

### Model Details
- **Student Model**: BERT-mini (11M parameters)
- **Teacher Model**: DistilBERT-base-uncased (66M parameters, 0.949 PII precision)
- **Training**: Knowledge distillation with temperature=2.0, 5 epochs
- **GPU Memory**: ~2.5GB (comfortably fits 4GB constraint)
- **Training Time**: ~7 minutes (including teacher + distillation)
...
### 1. Trained Models
- **Student Model** (FINAL): `out_student/` - BERT-mini (11M params)
- **Teacher Model**: `out_new/` - DistilBERT (66M params, used for distillation)

### 2. Predictions
- **Dev Set**: `out/dev_pred.json` (150 examples, from student model)
- **Test Set**: `out/test_pred.json` (100 examples, from student model)
...
- `distill.py` - Knowledge distillation script (teacher → student)
...
### Generate Predictions (Using Pre-trained Student Model)
```bash
# Dev set predictions (already generated)
python src/predict.py --model_dir out_student --input data/dev.jsonl --output out/dev_pred.json

# Test set predictions (already generated)
python src/predict.py --model_dir out_student --input data/test.jsonl --output out/test_pred.json
```
...
### Measure Latency
```bash
python src/measure_latency.py --model_dir out_student
```

### Train From Scratch (Optional)

#### Step 1: Train Teacher Model (4GB GPU)
```bash
python src/train.py \
...
#### Step 2: Knowledge Distillation (CPU/GPU)
```bash
python distill.py \
  --teacher_model out_new \
